...
const SessionDetailModal = dynamic(
  () => import('@/components/workout-plans/session-detail-modal').then(mod => mod.SessionDetailModal),
  { ssr: false }
)
...
export function DashboardContent({
  ...
  initialWeekSessions,
  initialCurrentWeek,
  initialWeeklyAnalysis,
}: DashboardContentProps) {
  const [workouts] = useState<Workout[]>(initialWorkouts)
  const [goals] = useState<Goal[]>(initialGoals)
  const [plans] = useState<WorkoutPlan[]>(initialPlans)
  const [weekSessions, setWeekSessions] = useState<WorkoutPlanSession[]>(initialWeekSessions)
  const [currentWeek] = useState<number>(initialCurrentWeek)
  const [selectedSession, setSelectedSession] = useState<WorkoutPlanSession | null>(null)
  const [modalOpen, setModalOpen] = useState(false)
  const [weeklyAnalysis, setWeeklyAnalysis] = useState<WeeklyAnalysis | null>(initialWeeklyAnalysis)

  async function refreshSessions() {
    try {
      const sessionsRes = await fetch('/api/workout-plans/sessions/current-week')
      if (sessionsRes.ok) {
                    ...
                    <div className="relative h-32 sm:h-40 w-full bg-muted overflow-hidden">
                      {selfie?.signedUrl ? (
                        <Image
                          ...
                          alt={selfie.caption || `Photo from ${workout.name} workout`}
                          fill
