...
class SyncStrategy(ABC):
    ...
    @abstractmethod
    async def calculate(self, ctx: SyncContext) -> Optional[float]:
...
class WorkoutCountStrategy(SyncStrategy):
    ...
    async def calculate(self, ctx: SyncContext) -> Optional[float]:
        response = ctx.supabase.table("workouts").select("id").eq(
            "user_id", ctx.user_id
...
class DurationStrategy(SyncStrategy):
    ...
    async def calculate(self, ctx: SyncContext) -> Optional[float]:
        response = ctx.supabase.table("workouts").select(
            "duration_minutes"
        ).eq("user_id", ctx.user_id).is_("deleted_at", "null").execute()
...
class UniqueDaysStrategy(SyncStrategy):
    ...
    async def calculate(self, ctx: SyncContext) -> Optional[float]:
        response = ctx.supabase.table("workouts").select("date").eq(
            "user_id", ctx.user_id
        ).is_("deleted_at", "null").execute()
...
class WeightStrategy(SyncStrategy):
    ...
    def _extract_exercise_name(self, title: str) -> Optional[str]:
    ...
    async def _get_body_weight(self, ctx: SyncContext) -> Optional[float]:
        """Get latest body weight from measurements."""
        response = ctx.supabase.table("body_measurements").select(
            "weight, weight_unit"
        ).eq("user_id", ctx.user_id).not_.is_(
            "weight", "null"
    ...
    async def _get_exercise_max_weight(
        ...
        response = ctx.supabase.table("exercises").select(
            "weight, weight_unit, workout_id"
        ).ilike("name", f"%{exercise_name}%").not_.is_(
            "weight", "null"
        ...
        workout_ids = [e.get("workout_id") for e in response.data if e.get("workout_id")]
        if not workout_ids:
            return None
        
        user_workouts_response = ctx.supabase.table("workouts").select("id").eq(
            "user_id", ctx.user_id
        ).in_("id", workout_ids).is_("deleted_at", "null").execute()
...
class DistanceStrategy(SyncStrategy):
    ...
    async def calculate(self, ctx: SyncContext) -> Optional[float]:
        ...
        # Build query
        query = ctx.supabase.table("exercises").select(
            "distance, distance_unit, workout_id, name"
...
def is_goal_achieved(initial_value: float, current_value: float, target_value: float) -> bool:
